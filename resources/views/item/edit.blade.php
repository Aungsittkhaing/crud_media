...
@section('content')
    <div class="w-full max-w-md mx-auto bg-white shadow-lg rounded-lg p-8">
        <form class="max-w-sm mx-auto" action="{{ route('item.update', $item->id) }}" method="POST">
            ...
            <div class="mb-5">
                <label for="price" class="block mb-2 text-sm font-medium text-gray-900">Your
                    Price</label>
                <input type="text"
                    class="
                @error('price')
                ...
                @enderror
                bg-gray-50 border text-gray-900 text-sm rounded-lg block w-full p-2.5"
                    placeholder="Price" name="price" value="{{ $item->price }}" />
                @error('price')
                    <span class="text-red-500">{{ $message }}</span>
                @enderror
            </div>
            <div class="mb-5">
                ...
                @enderror
                bg-gray-50 border text-gray-900 text-sm rounded-lg block w-full p-2.5"
                    placeholder="Stock" name="stock" value="{{ $item->stock }}" />
                @error('stock')
                    <span class="text-red-500">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-5">
                <div class="flex">
                    ...
                    form-radio text-blue-500" name="status"
                            value="available" {{ $item->status == 'available' ? 'checked' : '' }}>
            ...
            <div class="mb-5">
                ...
                block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border"
                    placeholder="Write your thoughts here..." name="description">{{ $item->description }}</textarea>
                @error('description')
                    <span class="text-red-500">{{ $message }}</span>
                @enderror
            </div>
